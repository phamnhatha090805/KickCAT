...
namespace kickcat
{
    ...
    {
        return (pdo.logical_start_bit  == 0) and (pdo.logical_stop_bit   == 7) and (pdo.physical_start_bit == 0);
    }
    ...
    uint32_t EmulatedESC::totalMappedBits(PDO const& pdo)
    {
        ...
        int64_t bits = int64_t(pdo.size) * 8 + int64_t(pdo.logical_stop_bit) - int64_t(pdo.logical_start_bit) - 7;
        if (bits <= 0)
        {
            return 0;
        }
    ...
    bool EmulatedESC::processBitAlignedPDO(DatagramHeader const* header, void* data, PDO const& pdo, bool read)
    {
        ...
        for (uint32_t bit = 0; bit < total_bits; ++bit)
        {
            ...
            if (read)
            {
                ...
                *frame_byte = static_cast<uint8_t>((*frame_byte & ~(1u << logical_bpos)) | (value << logical_bpos));
            }
            else
            {
                uint8_t value = (*frame_byte >> logical_bpos) & 0x1;
                *phys_byte = static_cast<uint8_t>((*phys_byte & ~(1u << physical_bpos)) | (value << physical_bpos));
            }
        }
        return touched;
    }
