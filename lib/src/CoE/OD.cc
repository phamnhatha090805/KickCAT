...
namespace kickcat::CoE
{
    ...
    void copyBits(uint8_t const* src, uint32_t src_bit_offset, uint8_t* dst, uint32_t dst_bit_offset, uint32_t n_bits)
    {
        ...
        // Fast path: byte-aligned source, destination and length
    ...
    {
        copyBits(static_cast<uint8_t const*>(entry->data), entry->data_bit_offset, dst, dst_bit_offset, entry->bitlen);
    }


    void writeEntryBits(Entry* entry, uint8_t const* src, uint32_t src_bit_offset)
    {
        copyBits(src, src_bit_offset, static_cast<uint8_t*>(entry->data), entry->data_bit_offset, entry->bitlen);
    }
