...
TEST(EsiParser, load_basic_with_bit_types_and_no_info)
{
    ...
    // 0x6000 - RECORD with BIT6 SubItem
    {
        ...
        // SubIndex 2: BOOL (DigitalIn) — sub-byte entry, must have at least 1 byte allocated
        ...
        // SubIndex 3: BIT6 (Padding) — sub-byte, must be allocated
        ...
        // Note: the ESI test fixture only declares DefaultData for 4 SubItems,
        // which the parser matches positionally to entries[0..3]; SI 4 has no
        // default value, so its data pointer is not allocated.
