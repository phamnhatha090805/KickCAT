...
{
    // 4 BOOL entries (BitLen=1) packed in one PDO byte. parsePdoMap must
    // record each entry's intra-byte bit position via Entry::data_bit_offset
    // so SDO upload/download can pick the right bit later.
    CoE::Dictionary dict;

    {
        CoE::Object obj{0x6000, CoE::ObjectCode::RECORD, "GPIOs", {}};
        // SubIndex 0 (count) lives in its own owned byte; not mapped through PDO.
    ...
    // All 4 BOOL entries must alias to input_[0], at successive bit positions.
    ...
    // Round-trip via the bit helpers: writing each entry independently must not
    // disturb its neighbours.
    auto [_, gpio1] = CoE::findObject(dict, 0x6000, 1);
    auto [__, gpio3] = CoE::findObject(dict, 0x6000, 3);

    uint8_t one = 0x01;
    CoE::writeEntryBits(gpio1, &one, 0); // GPIO1 = 1
    CoE::writeEntryBits(gpio3, &one, 0); // GPIO3 = 1

    EXPECT_EQ(input_[0] & 0x0F, 0x05); // bits 0 and 2 set, bits 1 and 3 clear
